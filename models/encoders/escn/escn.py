...
class eSCNEncoder(BaseModel):
    ...
    def __init__(
        ...
        # Output blocks for node embedding
        self.out_block = OutputBlock(
            self.sphere_channels_all, self.num_sphere_samples,
            self.hidden_channels, self.act
        )
...
class OutputBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        self.fc3 = nn.Linear(num_channels, out_channels, bias=False)
    ...
    def forward(self, x_pt) -> torch.Tensor:
        ...
        x_pt = self.fc3(x_pt)
        x_pt = x_pt.view(-1, self.num_sphere_samples, self.out_channels)
        node_emb = torch.sum(x_pt, dim=1) / self.num_sphere_samples
